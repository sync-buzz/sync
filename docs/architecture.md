...
**One thing travels the other way, and it is not a call.** A device may ask to watch a conversation an agent is holding, and what it is answered with is a number. Everything that conversation says afterwards arrives on the same connection under that number, as a notification: no id, no answer expected, and nothing on the far side that would run one. That is the whole of the reverse direction on a device's door — turning the connection around is still refused, for the reason above — and it is what makes an agent's answer appear on a phone as it is written rather than in one piece at the end.

The device says where to start: a watch carries the last thing that device has already been shown, so a connection that dropped is taken up again from there rather than replayed from the beginning. A conversation is watched by however many are watching it, and the machine running it is one of them — a device looking at a conversation does not take the live view away from the window at the desk. When a device's connection ends, the engine tells the application which watches went with it, because the application cannot see that far: it writes into a socket to a process that is still there.
